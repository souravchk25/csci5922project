#This is the file containing the training code
...
#setting Batch size, input/output resolution and GPU slot
vox_res64 = 64
vox_rex256 = 256
batch_size = 4
GPU0 = '0'
...
#########################
#setting the folder paths for training data and testing data
config={}
...
class Network:
    # This function creates the necessary folders for storing the trained model and necessary train/test results
    def __init__(self, demo_only=False):
        if demo_only:
            return  # no need to create folders
    ...
    # This function creates the generator layers
    # We are using 6 3D convolution layers for the encoder and 7 3D deconvolution layers for the decoder        
    def aeu(self, X):
        with tf.device('/gpu:'+GPU0):
            X = tf.reshape(X,[-1, vox_res64,vox_res64,vox_res64,1])
            c_e = [1,64,128,256,512,1024]
            s_e = [0,1 , 1, 1, 1, 1]
            layers_e = []
            layers_e.append(X)
            #Adding convolution layers with maxpool
            ...
            #Adding relu activations for each layer
        ...
        with tf.device('/gpu:'+GPU0):
            ...
            c_d = [0,512,256,128,64,16,8]
            s_d = [0,2,2,2,2,2,2]
            layers_d = []
            layers_d.append(lfc)
            #Adding de-convolution layers
            ...
            ###
            #Adding sigmoid activation for the final fully connected layer
            Y_sig = tf.nn.sigmoid(layer)
            Y_sig_modi = tf.maximum(Y_sig,0.01)

        return Y_sig, Y_sig_modi

    # This function creates the discriminator layers
    # A total of 7 3D convolution layers are added
    def dis(self, X, Y):
        with tf.device('/gpu:'+GPU0):
            X = tf.reshape(X,[-1, vox_res64, vox_res64, vox_res64,1])
            ...
            c_d = [1,8,16,32,64,128,256,512]
            s_d = [0,2,2,2,2,2,2,2]
            layers_d =[]
            layers_d.append(Y)
            #Adding convolution layers with relu activations
            ...
            # The final layer has sigmoid activation
        return tf.nn.sigmoid(y)

    #This function creates the model by initializing the Adam Optimizer for both generator and discriminator
    #This function is responsible for performing the gradient descent and computation of GAN loss
    #This function sets the Tensorflow session object to begin training
    #This function is called by the train method below
    def build_graph(self):
        ...
        with tf.device('/gpu:'+GPU0):
            ...
            self.aeu_g_optim = tf.train.AdamOptimizer(learning_rate=0.00001, beta1=0.9, beta2=0.999, epsilon=1e-8).\
                            minimize(self.aeu_gan_g_loss, var_list=aeu_var)
            self.dis_optim = tf.train.AdamOptimizer(learning_rate=0.000005, beta1=0.9, beta2=0.999, epsilon=1e-8).\
    ...
    #This function is responsible for shuffling data and start training for 15 epochs. Finally it saves the model    
    def train(self, data):
